...
class Model(object):
    ...
    def _compile_pytorch(self, lr, loss_fn, decay, loss_weights):
        ...
        def outputs_losses(training, inputs, targets):
            ...
            outputs_ = self.net(self.net.inputs.float())
            # Data losses
    ...
    def train(
        ...
        if optimizers.is_external_optimizer(self.opt_name):
            if backend_name == "tensorflow.compat.v1":
                ...
                self._train_pytorch_lbfgs(epochs=epochs)
        else:
            if epochs is None:
    ...
    def _train_pytorch_lbfgs(self, epochs=15000):
        prev_n_iter = 0
        while prev_n_iter < epochs:  # while prev_n_iter < optimizers.LBFGS_options["maxiter"]:
            self.callbacks.on_epoch_begin()
            self.callbacks.on_batch_begin()
